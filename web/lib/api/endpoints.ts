...
// ---------------------------------------------------------------------------
// Connectors (external source ingestion + ACL inheritance)
// ---------------------------------------------------------------------------
...
export const connectorsApi = {
  list: (workspaceId: string) =>
    api<ConnectorInstance[]>("/api/connectors", { workspaceId }),
  get: (workspaceId: string, id: string) =>
    api<ConnectorInstance>(`/api/connectors/${id}`, { workspaceId }),
  create: (
    workspaceId: string,
    data: {
      kind: string;
    ...
    api<ConnectorCreatedResponse>("/api/connectors", {
      method: "POST",
      body: data,
      workspaceId,
    }),
  oauthCallback: (
    ...
    api<{ ok: boolean; job_id: string }>(
      `/api/connectors/${id}/oauth-callback`,
      { method: "POST", body: payload, workspaceId },
    ),
  resync: (workspaceId: string, id: string) =>
    api<{ job_id: string }>(`/api/connectors/${id}/resync`, {
      method: "POST",
      workspaceId,
    }),
  remove: (workspaceId: string, id: string) =>
    api<void>(`/api/connectors/${id}`, {
      method: "DELETE",
      workspaceId,
    }),
};
...
// ---------------------------------------------------------------------------
// External identities (per-user bridge to source-system principals)
// ---------------------------------------------------------------------------
...
export interface ExternalIdentity {
  ...
  groups_resolution: "self" | "admin_sdk" | "none";
  created_at: string;
}

export const identityApi = {
  list: (workspaceId: string) =>
    api<ExternalIdentity[]>("/api/identity", { workspaceId }),
  googleAuthorizeUrl: (workspaceId: string) =>
    api<{ url: string }>("/api/identity/google/authorize-url", {
      method: "POST",
      workspaceId,
    }),
  googleCallback: (
    ...
    api<ExternalIdentity>("/api/identity/google/callback", {
      method: "POST",
      body: payload,
      workspaceId,
    }),
  remove: (workspaceId: string, id: string) =>
    api<void>(`/api/identity/${id}`, {
      method: "DELETE",
      workspaceId,
    }),
};
...
// ---------------------------------------------------------------------------
// Sources (connector-ingested episodes, ACL-filtered)
// ---------------------------------------------------------------------------
...
export interface SourceEpisodeDetail extends SourceEpisode {
...
export const sourcesApi = {
  list: (workspaceId: string, opts?: { limit?: number; offset?: number }) => {
    ...
    return api<SourceEpisode[]>(`/api/sources${suffix}`, { workspaceId });
  },
  get: (workspaceId: string, id: string) =>
    api<SourceEpisodeDetail>(`/api/sources/${id}`, { workspaceId }),
};
